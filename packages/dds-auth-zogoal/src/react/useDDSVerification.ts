  ...
  const initiateVerification = useCallback(
    async (mobileNumber: string) => {
      ...
      try {
        ...
        let res = await fetch(`${merchantApiUrl}/api/dds/verification/request`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ mobileNumber: canonical })
        })
        if (res.status === 404) {
          res = await fetch(`${merchantApiUrl}/api/auth/verify`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ mobileNumber: canonical })
          })
  ...
  useEffect(() => {
    if (status === 'pending' && requestId) {
      ...
      pollingRef.current = setInterval(async () => {
        try {
          let res = await fetch(`${merchantApiUrl}/api/dds/verification/status/${encodeURIComponent(requestId)}`)
          if (res.status === 404) {
            res = await fetch(`${merchantApiUrl}/api/auth/status/${encodeURIComponent(requestId)}`)
          }
          const data = await res.json()
